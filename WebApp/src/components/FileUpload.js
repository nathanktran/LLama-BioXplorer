...
const FileUpload = () => {
    ...
    const handlePrediction = async () => {
        ...
        try {
            ...
            const fullResponseText = response.data.prediction;
            let predictionText = []; // Default to an empty array


            // 1. Use a regex to find the line that looks like a list of strings.
            //    - `^`: Start of a line
            //    - `\['`: A literal opening bracket and single quote
            //    - `.*`: Any characters
            //    - `'\]`: A literal single quote and closing bracket
            //    - `$`: End of a line
            //    - `m`: Multi-line mode to make `^` and `$` work per line
            const matchList = fullResponseText.match(/^\['.*'\]$/m);


            // 2. Check if we found a matching line
            if (matchList && matchList[0]) {
                const listString = matchList[0];


                // 3. Extract the content inside the brackets: "['...']" -> "'...'"
                const innerContent = listString.slice(2, -2);


                // 4. Split by comma and clean up each term
                predictionText = innerContent.split(',')
            ...
            setPrediction([]); // Clear predictions on failure
        } finally {
            setLoading(false);
        }
    }
