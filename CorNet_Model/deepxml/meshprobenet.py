...
class MeSHProbes(nn.Module):    
    ...
    def forward(self, birnn_outputs, masks, return_attention=False):
        ...
        if return_attention:
            context_vectors, attention_weights = self.probes(birnn_outputs, masks, return_attention=True)
...
class MeSHProbeNet(nn.Module):
    ...
    def forward(self, input_variables, return_attention=False):
        emb_out, lengths, masks = self.emb(input_variables)
        birnn_outputs = self.lstm(emb_out, lengths)   # N, L, hidden_size * 2
        
        if return_attention:
            context_vectors, attention_weights = self.meshprobes(birnn_outputs, masks, return_attention=True)   
...
class CorNetMeSHProbeNet(nn.Module):
    ...
    def forward(self, input_variables, return_attention=False):
        if return_attention:
            raw_logits, attention_weights = self.meshprobenet(input_variables, return_attention=True)
            cor_logits = self.cornet(raw_logits)        
            return cor_logits, attention_weights
        else:
