...
from deepxml.evaluation import get_p_5, get_n_5, get_p_12, get_n_12
...
class Model(object):
    ...
    def train(self, train_loader: DataLoader, valid_loader: DataLoader, opt_params: Optional[Mapping] = None,
              nb_epoch=100, step=100, k=12, early=100, verbose=True, swa_warmup=None, **kwargs):
        self.get_optimizer(**({} if opt_params is None else opt_params))
        global_step, best_n12, e = 0, 0.0, 0
        print_loss = 0.0#
...
#                    labels = np.concatenate([self.predict_step(valid_x, k)[1] for valid_x in valid_loader])
                    ...
                    p12, n12 = get_p_12(labels, targets), get_n_12(labels, targets)
                    if n12 > best_n12:
                    ...
                    self.swap_swa_params()
                    if verbose:
                        log_msg = '%d %d train loss: %.7f valid loss: %.7f P@12: %.5f N@12: %.5f early stop: %d' % \
                        (epoch_idx, i * train_loader.batch_size, print_loss / step, valid_loss, round(p12, 5), round(n12, 5), e)
                        logger.info(log_msg)
                        print_loss = 0.0
    ...
    def predict_with_attention(self, data_loader, k=100):
        """
        Same as predict() but also returns attention weights for visualization
        """
        self.model.eval()
        scores, labels, attention_data = [], [], []
        
        with torch.no_grad():
            for inputs, _ in data_loader:
                inputs = inputs.cuda() if next(self.model.parameters()).is_cuda else inputs
                ...
                try:
                    ...
                    if hasattr(model_to_use, 'forward') and 'return_attention' in model_to_use.forward.__code__.co_varnames:
                        logits, attention_weights = model_to_use(inputs, return_attention=True)
                    else:
                        logits = self.model(inputs)
                except Exception as e:
                    # Fallback to regular prediction if attention extraction fails
                    logits = self.model(inputs)
                
                # Same prediction logic as original predict method
                batch_scores = torch.sigmoid(logits).cpu().numpy()
                batch_labels = np.argsort(-batch_scores, axis=1)[:, :k]
                ...
                if attention_weights is not None:
                    attention_data.extend(attention_weights.cpu().numpy())
                else:
                    attention_data.extend([None])
        
        return np.array(scores), np.array(labels), attention_data
